...
- Solid colors are cheaper to paint. 
- Gradients require per-pixel complex calculations. 
- Shadows and blurs are expensive because they examine multiple pixels to calculate each output pixel. 
- Text rendering is relatively optimized, while complex filters can be very costly.
